-- 1. Location of User 
SELECT * FROM post
WHERE location IN ('goa' ,'maharashtra','jaisalmer');


-- 2. Most Followed Hashtag
SELECT 
	hashtag_name AS 'Hashtags', COUNT(hashtag_follow.hashtag_id) AS 'Total_Follows' 
FROM hashtag_follow, hashtags 
WHERE hashtags.hashtag_id = hashtag_follow.hashtag_id
GROUP BY hashtag_follow.hashtag_id
ORDER BY Total_follows DESC LIMIT 5;

-- 3. Most Used Hashtags
SELECT 
	hashtag_name AS 'Trending_Hashtags', 
    COUNT(post_tags.hashtag_id) AS 'Times_Used'
...
SELECT 
    p.user_id, 
    p.post_id, 
    COUNT(p_likes.user_id) AS likes
FROM 
    post_likes p_likes
JOIN 
    post p ON p.post_id = p_likes.post_id
GROUP BY 
    p.user_id, p_likes.post_id
ORDER BY 
    likes DESC
LIMIT 10;
